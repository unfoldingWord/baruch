/**
 * UserQueue Durable Object for Baruch
 *
 * Simplified from bt-servant-worker:
 * - No injected KV fields (_mcp_servers, _org_config, _org_prompt_overrides, _org_modes)
...
const DEFAULT_STORED_RESPONSE_TTL_MS = 5 * 60 * 1000;
const DEFAULT_MAX_QUEUE_DEPTH = 50;
const DEFAULT_MAX_RETRIES = 3;
const DEFAULT_MAX_BUFFERED_RESPONSE_SIZE = 5 * 1024 * 1024;
const MAX_CLEANUP_PER_STORE = 10;
const ENQUEUE_RATE_WINDOW_MS = 60 * 1000;
const ENQUEUE_RATE_LIMIT = 300;
const POLL_RATE_WINDOW_MS = 60 * 1000;
const POLL_RATE_LIMIT_MULTIPLIER = 2;
...
export class UserQueue {
  ...
  private async handleEnqueue(request: Request): Promise<Response> {
    ...
    const response: EnqueueResponse = { message_id: result.message_id, queue_position: position };
  ...
  private async handleStream(url: URL): Promise<Response> {
    ...
    if (stored) return this.replayStoredResponse(stored);
  ...
  private async handlePoll(url: URL): Promise<Response> {
    ...
    const response: PollResponse = { message_id: messageId, events: [], done: false, cursor: 0 };
    return Response.json(response);
  }
  ...
  private async getAndClearStoredResponse(messageId: string): Promise<StoredResponse | null> {
    ...
    for (const [key, resp] of responses) {
      if (now - resp.stored_at > ttl) responses.delete(key);
    }
  ...
  private async processWithSSE(entry: QueueEntry, logger: RequestLogger): Promise<void> {
    ...
    if (writer) {
      logger.log('sse_client_connected', { message_id: entry.message_id });
    }
