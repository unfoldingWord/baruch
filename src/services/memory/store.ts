...
export class JsonMemoryStore implements UserMemoryStore {
  ...
  private evictToFit(data: MemoryStorage): string[] {
    ...
    if (totalSize > MAX_MEMORY_SIZE_BYTES) {
      this.truncateNewestEntry(data, totalSize);
    }

    return evicted;
  }

  /** Truncate the newest entry to fit within capacity when only pinned entries remain */
  private truncateNewestEntry(data: MemoryStorage, totalSize: number): void {
